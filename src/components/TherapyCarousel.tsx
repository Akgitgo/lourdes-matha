...
function Card({ item, icon }: { item: any; icon: React.ReactNode }) {
    return (
        <div className="bg-[#FAF3E0] rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden border border-[#B8860B]/10 shadow-lg hover:shadow-2xl transition-all duration-700 flex flex-col h-[600px] sm:h-[650px] lg:h-[750px] w-full group/card relative">
            {/* Image Section - Reduced to 65% for more text room */}
            ...
            {/* Floating Icon - Positioned at 65% */}
            <div className="absolute top-[65%] left-0 w-full flex justify-center -translate-y-1/2 z-20 pointer-events-none">
            ...
            {/* Text Section - Increased to 35% height */}
            <div className="px-5 sm:px-6 pb-6 sm:pb-8 pt-10 sm:pt-14 text-center h-[35%] flex flex-col justify-start items-center bg-[#FAF3E0] overflow-hidden">
                <h4 className="font-['Playfair_Display'] text-xl sm:text-lg lg:text-[22px] font-bold text-[#1a1a1a] mb-2 lg:mb-3 tracking-tight leading-tight group-hover/card:text-[#2d5a4f] transition-colors duration-300">
                    {item.title}
                </h4>
                <div className="flex-1 overflow-y-auto sm:overflow-hidden w-full custom-scrollbar">
                    <p className="font-['Inter'] text-sm sm:text-base text-[#4a4a4a] leading-relaxed font-medium opacity-90 px-2 lg:px-4">
                        {item.description}
                    </p>
                </div>
                {/* Visual anchor */}
